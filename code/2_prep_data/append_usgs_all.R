...
old_sites <- list.files('data/usgs_sm_ready_old/') %>% 
  ...
  data.frame() %>% 
  dplyr::mutate(sitecode = paste(X1,X2, sep = '_'))
...
long_sites <- dplyr::left_join(recent_sites, old_sites,
                               by = 'sitecode') %>% 
...
for(i in 1:length(long_sites$sitecode)) {
  ...
  old <- readr::read_csv(glue('data/usgs_sm_ready_old/{sitecode}_{old_date_range}')) %>% 
    select(site = site_id,
           solar.time,
           temp.water,
           DO.obs,
  ...
  recent <- readr::read_csv(glue('data/usgs_sm_ready_recent/{sitecode}_{recent_date_range}'))%>% 
    dplyr::select(site,
                  solar.time,
                  temp.water,
                  DO.obs,
  ...
  all <- rbind(old,recent) %>%    
    dplyr::distinct(solar.time, 
                    .keep_all = TRUE) 
  
  readr::write_csv(all,
                   glue::glue('data/usgs_sm_ready_all/{sitecode}_{substr(old_date_range, 1,10)}_{substr(recent_date_range, 12,25)}'))

  readr::write_csv(all,
                   glue::glue('data/data_citation/3_usgs_sm_ready_all/{sitecode}_{substr(old_date_range, 1,10)}_{substr(recent_date_range, 12,25)}'))
