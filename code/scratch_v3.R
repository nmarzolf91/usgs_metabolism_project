...
# are more productive sites more variable across years?
summary(lm(data = site_mean_ann_gpp, 
           median_ann_GPP~GPP_ann_cv))

# remove the outliers
summary(lm(data = site_mean_ann_gpp %>% 
             filter(sd_ann_GPP < 700), 
           median_ann_GPP~GPP_ann_cv))
...
compare_performance(cvQ_all, cvQ_by_site, cvQ_by_site_ranef)
...
compare_performance(temp_all, temp_by_site, temp_by_site_ranef)
